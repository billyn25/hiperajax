...
(function(){
  ...
  const SEARCH_ALIAS_RULES = Object.freeze([
    ...
    {rx:/\b(motion|movimiento|pir)\b/, add:'motion motionprotect motioncam movimiento pir detectores'},
  ...
  /* =====================================================
     CONTEXTO DE BÚSQUEDA DEL PROYECTO
     Explorer aporta familia, perfil y atajos reales.
     El motor común solo lo usa para desempatar resultados ya válidos.
     ===================================================== */
  let hxaSearchContextSignature = '';
  let hxaSearchContextByProduct = new WeakMap();

  function hxaEnsureSearchContextIndex(){
    const signature = productSignature();
    if(signature === hxaSearchContextSignature) return;

    hxaSearchContextByProduct = new WeakMap();
    buildModel().allItems.forEach(item => {
      if(item?.p && typeof item.p === 'object') hxaSearchContextByProduct.set(item.p, item);
    });
    hxaSearchContextSignature = signature;
  }

  function hxaSearchForms(value){
  ...
  function hxaSearchEquivalent(a,b){
    const left = hxaSearchForms(a);
    const right = hxaSearchForms(b);
  ...
  function hxaSearchTermMatches(term, contextWords, compactContext){
    const value = norm(term);
    if(!value) return false;

    if(contextWords.some(word =>
      hxaSearchEquivalent(value, word)
      || (value.length >= 3 && word.startsWith(value))
    )) return true;

    const compactValue = value.replace(/[^a-z0-9]/g,'');
    return compactValue.length >= 4 && compactContext.includes(compactValue);
  }

  function hxaSearchIntentRole(query){
    const value = norm(query);
    if(/\b(dummy|dummies|maqueta|maquetas|demo)\b/.test(value)) return 'dummy';
    if(/\b(kit|kits|bundle|pack)\b/.test(value)) return 'kit';
    if(/\b(soporte|soportes|bracket|brackets|mount|holder|carcasa|carcasas|cover|covers|tapa|tapas|repuesto|repuestos|recambio|recambios|accesorio|accesorios|pila|pilas|bateria|batería|baterias|baterías)\b/.test(value)) return 'accessory';
    return 'product';
  }

  function hxaSearchProductRole(item, family){
    const product = item?.p || {};
    const quickRole = quickProductRole(item);
    const profile = familyQuickProfile(family);

    const nameText = norm(product.name || '');
    const typeText = norm([
      item?.subcategory,
      product.subcategory,
    ...
    if(/\b(dummy|dummies|maqueta|maquetas|demo)\b/.test(`${nameText} ${typeText}`)) return 'dummy';

    if(quickRole.kit
      || /\b(kit|kits|bundle)\b/.test(`${nameText} ${typeText}`)
      || /\b(kits?|bundle)\b/.test(familyText)) return 'kit';

    const accessoryFamily =
      profile === 'spares'
      || /\b(repuesto|repuestos|recambio|recambios|soporte|soportes|pilas?|baterias?|baterías?)\b/.test(familyText)
      || (profile !== 'wireless_accessories'
        && /\b(accesorio|accesorios)\b/.test(familyText));

    const accessoryType =
      quickRole.accessory
      || /\b(soporte|support|bracket|mount|holder|carcasa|housing|cover|tapa|frame|marco|surfacebox|adaptador|adapter|repuesto|recambio)\b/.test(typeText);

    if(accessoryFamily || accessoryType) return 'accessory';
    return 'product';
  }

  function hxaSearchGroupRank(role, intentRole, direct){
    if(intentRole === 'product'){
      if(role === 'product') return direct ? 0 : 1;
      if(role === 'kit') return 2;
      if(role === 'accessory') return 3;
      return 4;
    }

    if(intentRole === 'kit'){
      return ({kit:0, product:1, accessory:2, dummy:3})[role] ?? 4;
    }

    if(intentRole === 'accessory'){
      return ({accessory:0, dummy:1, product:2, kit:3})[role] ?? 4;
    }

    return ({dummy:0, accessory:1, kit:2, product:3})[role] ?? 4;
  }

  function hxaExplorerProductContext(product, query){
    if(!product || typeof product !== 'object') return null;

    hxaEnsureSearchContextIndex();

    let item = hxaSearchContextByProduct.get(product);
    if(!item){
      const reference = norm(product.name || '');
      item = buildModel().allItems.find(candidate => norm(candidate.p?.name || '') === reference);
    }
    if(!item) return null;
    ...
    const contextWords = contextText.split(/\s+/).filter(Boolean);
    const compactContext = contextText.replace(/[^a-z0-9]/g,'');

    const queryText = norm(query);
    const queryWords = queryText.split(/\s+/).filter(Boolean);
    const aliasText = norm(searchAliases(query));
    const aliasStopWords = new Set([
      'ajax','producto','productos','accesorio','accesorios',
      'inalambrico','inalambricos','inalambrica','inalambricas',
      'smart','home','domotica','camara','camaras','cctv'
    ]);
    const aliasWords = aliasText.split(/\s+/)
      .filter(word => word.length >= 3 && !aliasStopWords.has(word));

    const directHits = queryWords.filter(term =>
      hxaSearchTermMatches(term, contextWords, compactContext)
    ).length;

    const aliasHits = aliasWords.filter(term =>
      hxaSearchTermMatches(term, contextWords, compactContext)
    ).length;

    const direct = directHits > 0 || aliasHits > 0;
    const role = hxaSearchProductRole(item, family);
    const intentRole = hxaSearchIntentRole(query);

    return {
      groupRank:hxaSearchGroupRank(role, intentRole, direct),
      affinity:(directHits * 100) + Math.min(aliasHits, 5) * 10,
      role,
      direct,
      family:family?.displayTitle || item.family || '',
      profile,
  ...
  window.HXA_EXPLORER_PRODUCT_CONTEXT = hxaExplorerProductContext;
